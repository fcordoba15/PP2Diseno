...
@WebServlet(urlPatterns = {"/RegistrosWebWeb"})
public class RegistrosWebWeb extends HttpServlet {
    ...
    if (Integer.parseInt(tipo) == 1){
        ...
        out.println("<hr>");
        out.println("<form method=\"get\" action=\"vistaWEB.xml\">\n" +"            \n" +"            <button>Abrir árbol XML</button>\n" +"        </form>");    
    }
    
    else if (Integer.parseInt(tipo) == 2){
        ConsultaBitacoraCt consultaCSV = new ConsultaBitacoraCt();
        ArrayList<String> datosCSV = consultaCSV.consultarPorVistaCSV("vistaWEB"); 
        ...
        ArrayList<String> datosTRAMAPLANA = consultaTRAMAPLANA.consultarPorVistaTramaPlana("vistaWEB"); 
